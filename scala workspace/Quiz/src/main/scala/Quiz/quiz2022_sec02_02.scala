...
object quiz2022_sec02_02 {
  ...
  def sameElemLst(l1:List[Any], l2:List[Any]):List[Any] = {
    if (l1.isEmpty) List()
    else if (isFound(l1.head, l2)) l1.head :: sameElemLst(l1.tail,l2)
    else sameElemLst(l1.tail,l2)
  }
  ...
  def moreThanHalf(l1:List[Any],l2:List[Any]):Boolean = {
    if (l1.isEmpty && l2.isEmpty) {
      ...
      val LstOfSameElem = sameElemLst(l1, l2)
