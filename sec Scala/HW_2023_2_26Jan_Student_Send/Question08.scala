...
object Question08 {
  def maxAll(lists:List[List[Int]]) :List[Int] = {
    if(lists.isEmpty) Nil
      ...
      var g = maxAll(lists.tail)
      if(lists.head.isEmpty) g
      else if(g.isEmpty) lists.head
      else if (lists.head.head > g.head) lists.head.head :: maxAll(List(lists.head.tail, g.tail))
      else g.head :: maxAll(List(lists.head.tail, g.tail))
    }
  }
